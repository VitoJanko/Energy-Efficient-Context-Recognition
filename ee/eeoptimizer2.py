...
class EnergyOptimizer:
    ...
    def find_sca_tradeoffs(self, binary_representation=False, name=None, cstree=False):
        ...
        if not binary_representation:
            tradeoffs = s2.sca_find_tradeoffs("enumerate", len(self.settings), len(self.contexts),
                                              self, NGEN=200, cstree=cstree)
            tradeoffs = self._encrypt_hof(tradeoffs)
        else:
            tradeoffs = s2.sca_find_tradeoffs("binary", len(self.settings[0]), len(self.contexts),
                                              self, NGEN=200, cstree=cstree)
    ...
    def dca_model(self, tradeoffs, active=1, setting=None, cf=None, max_cycle=None, energy_costs=None, name=None):
        ...
        prob, sleeping, working = d2.precalculate(self.transitions, max_cycle + 1, active)
        ...
        solutions = [d2.dca_evaluation(self.transitions, configuration, self.quality_metric, active=active,
    ...
    def find_dca_tradeoffs(self, max_cycle=10, active=1, seeded=True, setting=None, cf=None, name=None,
        ...
        def evl(x):
            d2.dca_evaluation(transitions=self.transitions, lengths=x, active=active, confusion=cf,
        ...
        tradeoffs = d2.dca_find_tradeoffs(len(self.contexts), max_cycle, evl, seeded=seeded, NGEN=ngen)
